...
// Step 2: Extract command-line tools zip using PowerShell
console.log(`Extracting ZIP to: ${sdkPath} using PowerShell...`);
try {
    const unzipCommand = `powershell -NoProfile -Command "Expand-Archive -Path '${zipPath}' -DestinationPath '${sdkPath}' -Force"`;
    execSync(unzipCommand, { stdio: 'inherit' });
    console.log(`Extracted ZIP to: ${sdkPath}`);
} catch (error) {
    console.error('Failed to extract ZIP with PowerShell:', error.message);
}
